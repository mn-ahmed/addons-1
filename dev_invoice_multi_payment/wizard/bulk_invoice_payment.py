...
class bulk_inv_payment(models.TransientModel):
    ...
    # payment_date = fields.Date('Payment Date', required="1", default=fields.Date.context_today)
    # communication = fields.Char('Memo',  store=True, readonly=False, compute='_compute_communication')
    ...
    def process_payment(self):
        ...
        for res in result:
            ...
            if not payment_method_id:
                payment_method_id = self.user_access_partner'account.payment.method'].search([],limit=1)
            payment_date = False
